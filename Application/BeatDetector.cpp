...
void BeatDetector::LoadSong(int sSize, const char* audioString)
{
	//Take in Aruguments
	sampleSize = sSize;
...
{
	currentTime = clock();
	currentTime = currentTime - initialTime - 500;
...
void BeatDetector::update()
{
		...
		if (started)
		{
			...
			if (specFlux > beatThreshold && (clock() - timeBetween) > 350)
			{
				...
				outfile << t->getMinutes() << ":" << t->getSeconds() << ":" << t->getMilliseconds() << std::endl;
			}
...
const char* BeatDetector::getSongName()
{
...
int BeatDetector::getCurrentTimeInt()
{
	return currentTime;
}
